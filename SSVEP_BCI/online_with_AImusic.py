...
def onlineSSVEP():
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() > 0:
            ...
            #     win.timeOnFlip(input_text, 'tStartRefresh')  # time at next scr refresh
            #     input_text.setAutoDraw(True)
            # if input_text.status == STARTED:
            #     # is it time to stop? (based on global clock, using actual start)
            #     if tThisFlipGlobal > input_text.tStartRefresh + cueTime - frameTolerance:
            #         # keep track of stop time/frame for later
            #         input_text.tStop = t  # not accounting for scr refresh
            #         input_text.frameNStop = frameN  # exact frame index
            #         win.timeOnFlip(input_text, 'tStopRefresh')  # time at next scr refresh
            #         input_text.setAutoDraw(False)
